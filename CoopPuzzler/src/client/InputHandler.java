...
public class InputHandler {
		...
		if (isWaiting) {
			...
			if (this.isTyping) {
				if ((Keyboard.isKeyDown(Keyboard.KEY_ESCAPE))
						|| (Mouse.isButtonDown(1)) || (Mouse.isButtonDown(0))) {
					this.isTyping = false;
					this.isWaiting = true;
				...
				if (!((coords[0] < 0) || (coords[1] < 0)
						|| (coords[1] >= this.mapNumRows) || (coords[0] >= this.mapNumColumns))) {
	...
	private void handleMouse() {
		this.zoomLevel = MouseHandler.updateZoomLevel(this.zoomLevel);
	}
}
